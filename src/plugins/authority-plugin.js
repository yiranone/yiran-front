 ...
 * @returns {boolean}
 */
const auth = function(authConfig, permission, role, permissions, roles, user) {
  const {check, type} = authConfig
  if (check && typeof check === 'function') {
    return check.apply(this, [permission, role, permissions, roles])
  }
  if (type === 'permission') {
    return checkFromPermission(check, permission, user)
  } else if (type === 'role') {
    return checkFromRoles(check, role)
  } else {
    return checkFromPermission(check, permission, user) || checkFromRoles(check, role)
  }
}
 ...
 * @returns {boolean}
 */
const checkFromPermission = function(check, permission, user) {
  return user && user.admin || permission && permission.operation && permission.operation.indexOf(check) !== -1
}
    ...
    Vue.mixin({
      ...
      methods: {
        ...
        $auth(check, type) {
          ...
          return auth.apply(this, [{check, type}, permission, role, permissions, roles, user])
        }
      }
    })
  }
