...
export default {
  ...
  methods: {
    ...
    handleUpdate (row, ids) {
      this.reset()
      const configId = row ? row.configId : ids[0]
      dataSource.configDetail({"configId":configId}).then(data => {
        this.form = data
        this.open = true
        this.formTitle = '修改参数'
      })
      ...
      this.$refs.form.validate(valid => {
        if (valid) {
          ...
          if (this.form.configId !== undefined) {
            dataSource.configEdit(this.form).then(data => {
            ...
            dataSource.configAdd(this.form).then(data => {
